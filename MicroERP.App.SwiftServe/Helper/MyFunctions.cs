...
namespace MicroERP.App.SwiftServe.Helper;
...
public static class MyFunctions
{
    ...
    public static async Task<T?> GetAsync<T>(string url, bool useTokenAuthorize = false)
    {
        try
        {
            ...
            if (useTokenAuthorize && !string.IsNullOrEmpty(MicroERP.App.SwiftServe.Helper.MyGlobals.Token))
            {
                httpClient.DefaultRequestHeaders.Authorization =
                    new AuthenticationHeaderValue("Bearer", MicroERP.App.SwiftServe.Helper.MyGlobals.Token);
            }
    ...
    public static async Task<(bool Success, T? Result, string Message)> PostAsync<T>(string url, object? data = null, bool useTokenAuthorize = false)
    {
        try
        {
            ...
            if (useTokenAuthorize && !string.IsNullOrEmpty(MicroERP.App.SwiftServe.Helper.MyGlobals.Token))
            {
                httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", MicroERP.App.SwiftServe.Helper.MyGlobals.Token);
            }
