...
public static class GLAccountLookupHelper
{
    ...
    /// <summary>
    /// Get AR/AP account: Invoice.AccountId (must match AR for sale / AP for purchase) else Party.AccountId (same type). No default pick; error if none defined. Same logic as CreateGLFromInvoice.
    /// </summary>
        ...
        int? partyId = null)
    {
        ...
        // Priority 2: Get AccountId from Party — accept the account linked on the Party if it exists and is active (no InterfaceType required; party link is explicit)
        if ((arApAccount == null || arApAccount.Id == 0) && partyId.HasValue && partyId.Value > 0)
        {
            string sqlPartyAccount = $@"SELECT TOP 1 coa.Id, coa.Code, coa.Name, coa.Type, coa.InterfaceType
                                        FROM Parties p
        ...
        // No fallback: if no account defined on Invoice or Party, return error — do not pick any account
        if (arApAccount == null || arApAccount.Id == 0)
        {
            string errorMsg = $"No {accountTypeName} account defined. Set the account on the Invoice (with InterfaceType '{expectedInterfaceType}') or link the Party to an account in Chart of Accounts (Party → AccountId). No default account will be used.";
        ...
        int? partyId = null)
    {
        glHeader.Details.Add(new GeneralLedgerDetailModel
        {
            ...
            PartyId = partyId.HasValue ? Convert.ToInt32(partyId.Value) : 0
        });
    }
