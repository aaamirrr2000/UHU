    ...
    public async Task<(bool, InvoiceModel, string)> Post(InvoicesModel obj)
    {
        try
        {
            ...
            foreach (var detail in obj.InvoiceDetails ?? new ObservableCollection<InvoiceItemReportModel>())
            {
                ...
                int accountIdVal = GetIntProp(detail, "AccountId");
                if (accountIdVal > 0) detailAccountId = accountIdVal;

                // Convert ItemId = 0 to NULL for manual entries (foreign key constraint)
                string itemIdValue = itemId > 0 ? itemId.ToString() : "NULL";
                string accountIdValue = detailAccountId.HasValue ? detailAccountId.Value.ToString() : "NULL";

                string detailInsert = $@"
                    INSERT INTO InvoiceDetail (ItemId, AccountId, StockCondition, ManualItem, ServingSize, Qty, UnitPrice, DiscountAmount,
                    InvoiceId, Description, Status, Rating, TranDate, IsSoftDeleted)
                    VALUES ({itemIdValue}, {accountIdValue}, '{stockCond}', '{manualItem}', '{serving}', {qty.ToString(CultureInfo.InvariantCulture)},
        ...
        try
        {
            ...
            if (details.Count > 0)
            {
                ...
                foreach (var detail in details)
                {
                    ...
                    detailValues.Add($"({itemIdValue}, {accountIdValue}, '{stockCond}', '{manualItem}', '{serving}', {qty.ToString(CultureInfo.InvariantCulture)}, " +
