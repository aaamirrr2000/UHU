...
public class Globals
{
    ...
    /// <summary>Check access by page name only.</summary>
    public int PageAccess(NavigationManager? navManager, string pageName)
        => PageAccess(navManager, pageName, parameter: null);

    /// <summary>Check access by page name and optional parameter (e.g. InvoiceDashboard + SALE or PURCHASE). When parameter is set, permission must match both PageName and Parameter.</summary>
    public int PageAccess(NavigationManager? navManager, string pageName, string? parameter)
    {
        try
        {
            ...
            string? normalizedParam = string.IsNullOrWhiteSpace(parameter) ? null : parameter.Trim().ToUpperInvariant();

            var menuPermission = MyPermissions.FirstOrDefault(p =>
                p.PageName?.Trim().ToLower() == normalizedPage &&
                p.GroupId == User.GroupId &&
                p.IsActive == 1 &&
                (normalizedParam == null || (p.Parameter?.Trim().ToUpperInvariant() == normalizedParam)));
