...
public class LeavesService : ILeavesService
{
    ...
    public async Task<(bool, string)> Put(LeavesModel obj)
    {
        try
        {   string SQLUpdate = $@"UPDATE Leaves SET 
					...
					FromDate = '{DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss")}', 
					ToDate = '{DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss")}', 
					Description = '{obj.Description!.ToUpper()}', 
					Approved = '{DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss")}', 
