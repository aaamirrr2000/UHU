...
{
    InvoiceDetailService Srv = new InvoiceDetailService();
    ...
    [HttpPost("InvoiceItemStatus/{Id}/{Status}/{SoftDelete?}")]
    public async Task<IActionResult> InvoiceItemStatus(int Id, string Status, int SoftDelete=0)
    {
        var result = await Srv.InvoiceItemStatus(Id, Status, SoftDelete)!;
        if (result.Item1 == true)
            return Ok(result.Item2);
        else
            return BadRequest(result.Item2);
